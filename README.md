...
> 점수 순위와 실전 활용 순위는 다르다. 각 모델의 강점을 단계별로 나눠 쓰는 것이 핵심.

| 단계 | 상황 | 추천 모델 |
|------|------|-----------|
| 브레인스토밍 | 내가 도메인 전문가 → 내 의견을 많이 담고 싶을 때 | **Claude Code Opus** |
| 브레인스토밍 | AI 의견을 최대한 활용하고 싶을 때 | **Claude Code Sonnet** |
| 구현 | 기존 코드베이스 컨벤션을 일관되게 유지해야 할 때 | **Codex** |
| 스킬 관리 | 체계적인 스킬 인프라가 필요할 때 | **Gemini CLI** |

**단계별 조합 예시:** 설계(Opus/Sonnet) → 구현(Codex)

→ 상세 분석: [`results/comparison-summary.md`](results/comparison-summary.md)

## 워크플로우 평가 점수 (참고)

> 이 점수는 "2W1H 워크플로우 프로세스 이행 능력"만을 측정한 것으로, 모델의 전체 활용 가치와는 다르다.
> (예: Codex는 코드 컨벤션 일관성이라는 실전 강점이 점수에 반영되지 않음)
